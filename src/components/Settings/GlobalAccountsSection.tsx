import { useState, useEffect, useCallback } from 'react';
import { useSearchParams } from 'react-router-dom';
import { Button } from '../common';
import type { BoardCredential } from '../../types';
import * as api from '../../api/client';
import './AccountsSection.css';

/**
 * Account definitions for the UI
 * These match the accounts in worker/mcp/AccountMCPRegistry.ts
 */
const ACCOUNTS = [
  {
    id: 'google',
    name: 'Google',
    credentialType: 'google_oauth',
    description: 'Gmail, Google Docs, and Sheets',
    icon: (
      <svg viewBox="0 0 24 24">
        <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
        <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
        <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
        <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
      </svg>
    ),
  },
  {
    id: 'github',
    name: 'GitHub',
    credentialType: 'github_oauth',
    description: 'Repository access for code changes',
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor">
        <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
      </svg>
    ),
  },
];

export function GlobalAccountsSection() {
  const [searchParams, setSearchParams] = useSearchParams();
  const [credentials, setCredentials] = useState<BoardCredential[]>([]);
  const [connecting, setConnecting] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  // Check for OAuth success/error in URL params
  useEffect(() => {
    const googleConnected = searchParams.get('google');
    const googleError = searchParams.get('google_error');
    const githubConnected = searchParams.get('github');
    const githubError = searchParams.get('github_error');

    if (googleConnected === 'connected' || githubConnected === 'connected') {
      loadCredentials();
      searchParams.delete('google');
      searchParams.delete('github');
      setSearchParams(searchParams, { replace: true });
    }

    if (googleError) {
      setError(`Google connection failed: ${googleError}`);
      searchParams.delete('google_error');
      setSearchParams(searchParams, { replace: true });
    }

    if (githubError) {
      setError(`GitHub connection failed: ${githubError}`);
      searchParams.delete('github_error');
      setSearchParams(searchParams, { replace: true });
    }
  }, [searchParams, setSearchParams]);

  const loadCredentials = useCallback(async () => {
    const result = await api.getGlobalCredentials();
    if (result.success && result.data) {
      setCredentials(result.data);
    }
  }, []);

  useEffect(() => {
    loadCredentials();
  }, [loadCredentials]);

  const handleConnect = async (provider: string) => {
    setConnecting(provider);
    setError(null);

    try {
      const result = await api.getGlobalOAuthUrl(provider);
      if (result.success && result.data?.url) {
        window.location.href = result.data.url;
      } else {
        setError(result.error?.message || 'Failed to get OAuth URL');
        setConnecting(null);
      }
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Failed to connect');
      setConnecting(null);
    }
  };

  const handleDisconnect = async (credentialId: string) => {
    const result = await api.deleteGlobalCredential(credentialId);
    if (result.success) {
      setCredentials((prev) => prev.filter((c) => c.id !== credentialId));
    } else {
      setError(result.error?.message || 'Failed to disconnect');
    }
  };

  const getCredential = (credentialType: string) =>
    credentials.find((c) => c.type === credentialType);

  const getConnectionInfo = (cred: BoardCredential) => {
    if (cred.metadata?.email) return cred.metadata.email as string;
    if (cred.metadata?.login) return cred.metadata.login as string;
    return 'Connected';
  };

  return (
    <div className="accounts-section">
      {error && <div className="accounts-error">{error}</div>}

      {ACCOUNTS.map((account) => {
        const credential = getCredential(account.credentialType);
        const isConnecting = connecting === account.id;

        return (
          <div
            key={account.id}
            className={`account-card ${credential ? 'connected' : ''}`}
          >
            <div className="account-card-left">
              <div className="account-card-icon">{account.icon}</div>
              <div className="account-card-info">
                <span className="account-card-name">{account.name}</span>
                <span className="account-card-meta">
                  {credential
                    ? getConnectionInfo(credential)
                    : account.description}
                </span>
              </div>
            </div>
            <div className="account-card-actions">
              {credential ? (
                <button
                  className="account-disconnect-btn"
                  onClick={() => handleDisconnect(credential.id)}
                  title="Disconnect"
                >
                  Disconnect
                </button>
              ) : (
                <Button
                  variant="default"
                  size="sm"
                  onClick={() => handleConnect(account.id)}
                  disabled={isConnecting}
                >
                  {isConnecting ? 'Connecting...' : 'Connect'}
                </Button>
              )}
            </div>
          </div>
        );
      })}
    </div>
  );
}
